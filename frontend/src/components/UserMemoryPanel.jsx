...
export default function UserMemoryPanel({ memory, className = "" }) {
  ...
  return (
    <div
      className={`rounded-xl border border-slate-700/60 bg-slate-900/35 p-3 text-xs text-slate-400 ${className}`}
    >
      ...
      <div className="mt-2">
        ...
        {topics.length ? (
          ...
          <span className="text-slate-600">появятся по мере занятий</span>
        )}
      </div>
          ...
          <ul className="mt-1 space-y-1 text-slate-400">
            {mistakes.slice(-6).map((m, i) => (
              <li key={`${m.topic ?? ""}-${m.error ?? ""}-${i}`}>
                <span className="text-slate-600">{m.topic ?? "—"}:</span> {m.error}
              </li>
            ))}
          </ul>
        </div>
